...
public class SlimeOre extends Block {
    public SlimeOre(final Material mat) {
        super(mat);
        this.setBlockName("SlimeOre").setHardness(1.2f).setResistance(30.0f);
        this.setStepSound(Block.soundTypeStone).setHarvestLevel("pickaxe", 2);
        this.setCreativeTab(CreativeTabs.tabBlock);
        this.setBlockTextureName("galaxycore:slime_ore");
    }

    public Item getItemDropped(final int meta, final Random random, final int fortune) {
        return LoaderItems.SlimeIngot;
    }
